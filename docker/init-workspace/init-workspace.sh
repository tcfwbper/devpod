...
if [ ! -f "/tmp/workspace/.bashrc" ]; then
    ...
    echo "export REDIS_HOST=\"$DEVELOPER-$ENVIRONMENT-redis-svc.$NAMESPACE.svc.cluster.local\"" >> /tmp/workspace/.bashrc
    echo "export POSTGRES_HOST=\"$DEVELOPER-$ENVIRONMENT-postgresql-svc.$NAMESPACE.svc.cluster.local\"" >> /tmp/workspace/.bashrc
    echo "export MSSQL_HOST=\"$DEVELOPER-$ENVIRONMENT-mssql-svc.$NAMESPACE.svc.cluster.local\"" >> /tmp/workspace/.bashrc
    if [ ! -z "${CLOUD}" ]; then
        echo "export DOCKER_HOST=\"tcp://docker-daemon-svc.docker-daemon.svc.cluster.local:2375\"" >> /tmp/workspace/.bashrc
    fi
fi
# bash_profile
